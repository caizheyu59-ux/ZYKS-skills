# Prompt Construction

## Principles

Good prompts must include:

1. **Layout Structure First**: Describe composition, zones, flow direction
2. **Specific Data/Labels**: Use actual numbers, terms from article (IN CHINESE)
3. **Visual Relationships**: How elements connect
4. **Semantic Colors**: Meaning-based color choices (red=warning, green=efficient)
5. **Style Characteristics**: Line treatment, texture, mood
6. **Aspect Ratio**: End with ratio and complexity level
7. **Chinese Language**: ALL text, labels, and annotations MUST be in Chinese

## Type-Specific Templates

### Infographic

```
[Title] - Data Visualization

Layout: [grid/radial/hierarchical]

ZONES:
- Zone 1: [data point with specific values]
- Zone 2: [comparison with metrics]
- Zone 3: [summary/conclusion]

LABELS: [specific numbers, percentages, terms from article - IN CHINESE]
COLORS: [semantic color mapping]
STYLE: [style characteristics]
LANGUAGE: All text and labels must be in Chinese
ASPECT: 16:9
```

### Scene

```
[Title] - Atmospheric Scene

FOCAL POINT: [main subject]
ATMOSPHERE: [lighting, mood, environment]
MOOD: [emotion to convey]
COLOR TEMPERATURE: [warm/cool/neutral]
STYLE: [style characteristics]
LANGUAGE: Any text must be in Chinese
ASPECT: 16:9
```

### Flowchart

```
[Title] - Process Flow

Layout: [left-right/top-down/circular]

STEPS:
1. [Step name in Chinese] - [brief description in Chinese]
2. [Step name in Chinese] - [brief description in Chinese]
...

CONNECTIONS: [arrow types, decision points]
STYLE: [style characteristics]
LANGUAGE: All step labels and descriptions in Chinese
ASPECT: 16:9
```

### Comparison

```
[Title] - Comparison View

LEFT SIDE - [Option A in Chinese]:
- [Point 1 in Chinese]
- [Point 2 in Chinese]

RIGHT SIDE - [Option B in Chinese]:
- [Point 1 in Chinese]
- [Point 2 in Chinese]

DIVIDER: [visual separator]
STYLE: [style characteristics]
LANGUAGE: All comparison points in Chinese
ASPECT: 16:9
```

### Framework

```
[Title] - Conceptual Framework

STRUCTURE: [hierarchical/network/matrix]

NODES:
- [Concept 1 in Chinese] - [role in Chinese]
- [Concept 2 in Chinese] - [role in Chinese]

RELATIONSHIPS: [how nodes connect]
STYLE: [style characteristics]
LANGUAGE: All concept labels in Chinese
ASPECT: 16:9
```

### Timeline

```
[Title] - Chronological View

DIRECTION: [horizontal/vertical]

EVENTS:
- [Date/Period 1]: [milestone in Chinese]
- [Date/Period 2]: [milestone in Chinese]

MARKERS: [visual indicators]
STYLE: [style characteristics]
LANGUAGE: All event labels in Chinese
ASPECT: 16:9
```

## What to Avoid

- Vague descriptions ("a nice image")
- Literal metaphor illustrations
- Missing concrete labels/annotations
- Generic decorative elements
- **English text or labels - ALL text must be in Chinese**

## Watermark Integration

If watermark enabled in preferences, append:

```
Include a subtle watermark "[content]" positioned at [position] with approximately [opacity*100]% visibility.
```
